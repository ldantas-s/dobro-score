...
<template>
  <div v-show="game.isIdle()" class="flex flex-col gap-2">
    <input
      type="text"
      data-test="input-player"
      class="rounded-2 text-black p-16 text-center bg-white bold-400"
      ref="playerName"
    />
    <Button
      data-test="add-player__button"
      @click="addPlayer"
      :disabled="game.hasLimitPlayers()"
    >
      Adicionar Jogador
    </Button>
  </div>
